export default async function handler(req, res) {
  ...
  try {
    ...
    function parseZ(arr) {
      const r = [];
      for (let i = 0; i < (arr||[]).length; i += 2) r.push([arr[i], parseInt(arr[i+1])||0]);
      return r;
    }
    return res.status(200).json({
      stats: {
        total_sessions: ts, total_assessments: ta, total_concours_views: tcv,
        active_this_week: Math.floor(ts * 0.3),
        top_careers: parseZ(rc), top_concours: parseZ(rco),
        top_fields: parseZ(rf), streams: parseZ(rs),
      },
